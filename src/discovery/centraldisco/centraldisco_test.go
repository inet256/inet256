...
func TestClientServer(t *testing.T) {
	...
	c1 := NewClient(gc)
	c2 := NewClient(gc)
	pk1 := inet256tests.NewPrivateKey(t, 0)
	pk2 := inet256tests.NewPrivateKey(t, 1)
	...
	require.NoError(t, err)
	// find
	endpoints, err := c1.Lookup(ctx, inet256.NewID(pk1.Public().(inet256.PublicKey)))
	require.NoError(t, err)
	require.Len(t, endpoints, 1)
	endpoints, err = c2.Lookup(ctx, inet256.NewID(pk2.Public().(inet256.PublicKey)))
	require.NoError(t, err)
	require.Len(t, endpoints, 1)
