...
func NewTestServer(t testing.TB, nf NetworkFactory) *Server {
	...
	t.Cleanup(cf)
	pk := inet256tests.NewPrivateKey(t, math.MaxInt32)
...
func NewTestServers(t testing.TB, nf NetworkFactory, xs []inet256.Service) {
	...
	for i := range srvs {
		priv := inet256tests.NewPrivateKey(t, math.MaxInt32+i)
		pubX509 := PublicKeyFromINET256(priv.Public().(inet256.PublicKey))
